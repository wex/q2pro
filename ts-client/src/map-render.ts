...
export function generateMapSvg(
  ...
  if (worldW <= 0 || worldH <= 0) {
    throw new Error('World bounds are degenerate');
  }
...
const isCLI = process.argv[1]?.endsWith('map-render.ts') || process.argv[1]?.endsWith('map-render.js');
if (isCLI) {
  ...
  const cliShowGrid = args.includes('--grid');
