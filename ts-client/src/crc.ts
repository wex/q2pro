/**
 * CRC and checksum utilities matching Q2's COM_BlockSequenceCRCByte.
 * Ported from src/common/crc.c
 */
...
function crcBlock(data: Uint8Array, length: number): number {
  let crc = CRC_INIT_VALUE;
...
/**
 * Compute the Q2 move command checksum byte.
 * Matches COM_BlockSequenceCRCByte from src/common/crc.c
 */
export function blockSequenceCRCByte(base: Buffer, offset: number, length: number, sequence: number): number {
  const p = sequence % (chktbl.length - 4);

  const actualLen = Math.min(length, 60);
  const chkb = new Uint8Array(actualLen + 4);

  for (let i = 0; i < actualLen; i++) {
    chkb[i] = base[offset + i];
  }

  chkb[actualLen] = chktbl[p];
  chkb[actualLen + 1] = chktbl[p + 1];
  chkb[actualLen + 2] = chktbl[p + 2];
  chkb[actualLen + 3] = chktbl[p + 3];

  const totalLen = actualLen + 4;
  let crc = crcBlock(chkb, totalLen);
